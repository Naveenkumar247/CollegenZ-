...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // ------------------------------
      if (is_featured === "true" && isAdmin) {
        postData.isFeatured = true;
        postData.featuredOrder = Number(featured_order) || 0;

        if (featured_until) {
          postData.featuredUntil = new Date(featured_until);
        }
      }
      ...
      console.log(`✔ Post saved (${post_type.toUpperCase()})`);
...
<header class="header" id="mainHeader">
  ...
  <!-- TOP HEADER (HIDES) -->
  <a href="/" class="doodle top-header" id="topHeader">
    <img src="Doodle.png" alt="CollegenZ Logo" class="logo">
  </a>

  <!-- BOTTOM HEADER (ALWAYS VISIBLE) -->
  <div class="header-row bottom-header" id="bottomHeader">
...
window.addEventListener("scroll", () => {
    ...
    const showAfter = window.innerHeight * 0.25;
